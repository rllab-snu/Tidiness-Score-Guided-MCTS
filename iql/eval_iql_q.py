...
from src.value_functions import TransportQ, ResNetTwinQ
    ...
    with tqdm(test_loader) as bar:
        for i, data in enumerate(bar, 0):
            ...
            for b in range(args.batch_size):
                ...
                plt.subplot(1, 3, 2)
                plt.imshow(q[b]) #, vmin=0., vmax=1.)
                texts = []
                for _y in range(q[b].shape[0]):
                    for _x in range(q[b].shape[1]):
                        txt = plt.text(_x, _y, '%.1f'%q[b][_y, _x], ha='center', va='center')
                        # if q[b][_y, _x] > 0.001:
                        #     txt = plt.text(_x, _y, '%.2f'%q[b][_y, _x], ha='center', va='center')
                        # else:
                        #     txt = plt.text(_x, _y, '%.1f'%q[b][_y, _x], ha='center', va='center')
                        texts.append(txt)
                plt.subplot(1, 3, 3)
                plt.imshow(labels.cpu().detach().numpy()[b], vmin=0., vmax=1.)
                plt.savefig(os.path.join(args.log_dir, log_name, 'q-%d.png'%count_steps))
                for txt in texts:
                    txt.remove()
